...
class TestBlockGridSpaceManager(unittest.TestCase):
	def setUp(self):
		...
		self.bgss = block_grid_space_manager.BlockGridSpaceSpecification(self.start_x, self.start_y, self.block_width, self.block_height, self.height_modifier)
	...
	def testCalculateBlockPoints(self):
		...
		result = block_grid_space_manager.calculate_block_points((0, 0), block, self.bgss)
